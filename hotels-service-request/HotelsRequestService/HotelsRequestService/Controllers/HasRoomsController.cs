...
using static HotelsRequestService.DTOs.HasRoomDTOs;

namespace HotelsRequestService.Controllers
{
    [Route("api/Hotels/{id}/rooms")]
    [ApiController]
    public class HasRoomsController : ControllerBase
    {
        private readonly ApiDbContext _context;
        private readonly IMapper _mapper;

        public HasRoomsController(IMapper mapper, ApiDbContext context)
        {
            _mapper = mapper;
            _context = context;
        }
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<HasRoomResponseDTO>>> GetRooms(int id)
        {
            ...
            var roomsDTO = _mapper.Map<List<HasRoomResponseDTO>>(hotel.HasRooms);
        ...
        [HttpGet("{roomId}")]
        public async Task<ActionResult<HasRoomResponseDTO>> GetRoom(int id, int roomId)
        {
            var room = await _context.HasRooms.FindAsync(roomId);
            if (room == null) { return NotFound(); }

            var roomDTO = _mapper.Map<HasRoomResponseDTO>(room);
        ...
        [HttpPut("{roomId}")]
        public async Task<IActionResult> PutRoom(int id, int roomId, HasRoomCreateDTO roomDTO)
        {
            var room = await _context.HasRooms.FindAsync(roomId);
            if (room == null) { return NotFound(); }
            ...
            {
                if (!RoomExists(roomId)) { return NotFound(); }
                else { throw; }
            }
        ...
        [HttpPost]
        public async Task<ActionResult<HasRoomResponseDTO>> PostRoom(int id, HasRoomCreateDTO roomDTO)
        {
            var hotel = await _context.Hotels.FindAsync(id);
            if (hotel == null) { return NotFound(); }

            var room = _mapper.Map<HasRoom>(roomDTO);
            room.Hotel = hotel;

            _context.HasRooms.Add(room);
            await _context.SaveChangesAsync();

            var roomDetailsDTO = _mapper.Map<HasRoomResponseDTO>(room);
        ...
        {
            return _context.HasRooms.Any(e => e.Id == id);
        }
