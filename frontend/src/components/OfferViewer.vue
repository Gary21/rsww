...
methods: {
    ...
    async fetchData() {
        ...
        if(offer.hasFood == 1)
            ...
            this.hasFood = "no";
        const response1 = await fetch('http://localhost:8080/Offers/GetTrips?HotelId=' + this.$route.query.id + '&Date=' + this.$route.query.date + '&PeopleNumber=' + (this.$route.query.adult+this.$route.query.child18+this.$route.query.child10+this.$route.query.child3));
        var trip = await response1.json();
    ...
    async init_websocket() {
      var socket_url = 'ws://localhost:8080/Offers/HotelWebsocket?hotelId=' + this.$route.query.id;
    ...
    onSocketMessage(evt){
      ...
      if(received == "UNAVAILABLE" || received == "PURCHASE" || received == "RESERVE")
      {
          this.text = received;
          this.snackbar = true;
      }
